#!/bin/bash
# Runs on every boot via presence-firstboot.service.
# If WiFi is connected, exits cleanly so the engine + PWA start normally.
# If not connected, enters AP mode and launches the setup portal.
...
# Start the setup portal (port 80 requires root)
